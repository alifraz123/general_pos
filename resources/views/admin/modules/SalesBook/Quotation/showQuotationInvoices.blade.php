              ...
              <tr>
                ...
                <td>
                  <a style="color: white;width:100%;position:absolute">
                    <img onclick="showHide('showHideDiv{{$saleInvoice->Invoice}}')" src="images/tripledot.png"
                      style="width: 20px; height:20px;cursor:pointer" alt="">

                    <div
                      style="cursor:pointer;position: relative;top:0px;left:0px;display:none;z-index:1000;padding:10px;background-color:white;color:black"
                      class="showHideDiv{{$saleInvoice->Invoice}}">
                      <p
                        onclick="editFunction('edit_invoice?id={{$saleInvoice->Invoice}}','showHideDiv{{$saleInvoice->Invoice}}')">
                        Edit
                      </p>

                      <p class="dropdown-item "
                        onclick="confirmToDelete('delete_quotation?startDate={{$saleInvoice->Date}}&endDate={{$saleInvoice->Date}}&Invoice={{$saleInvoice->Invoice}}')">
                        Delete
                      </p>
...
<script>
  function showHide(value) {
    ...
    function editFunction(value,editId){
      ...
      window.open(value + '&Ref=QT', '_blank');
    }
